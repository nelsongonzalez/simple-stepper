...
export class SimpleStepper extends LitElement {
  ...
  updated(changedProps) {
    ...
    if (horizontalChanged && this._steps) {
      for (let step of this._steps) step._horizontal = this.horizontal;
    }
  }
  ////////////////// Public functions //////////////////
  /**
   ...
   */
  closeAll() {
    for (let step of this._steps) step.active = false;
  }
  /**
  ...
  _finishStep() {
    ...
    this._steps
      .filter(step => step._stepIndex !== currStep._stepIndex)
      .forEach(step => {
        console.log('checking step:', step.optional, step.save, step._stepIndex, step.locked);
        if (!step.locked && !step.optional && !step.save) {
    ...
    if (hasInvalid || currStep.invalid) {
      ...
      this.finish = true;
      this.dispatchEvent(
        new CustomEvent('stepper-finished', {
          bubbles: true,
          composed: true,
        }),
      );
